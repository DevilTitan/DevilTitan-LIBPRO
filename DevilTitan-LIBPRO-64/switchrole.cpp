    ...
    ui(new Ui::SwitchRole)
{
    ...
    this->setWindowTitle("Switch Role");
}
...
void SwitchRole::on_reader_clicked()
{
    ...
    this->ui->librarian->setStyleSheet("background-color: rgba(12, 60, 85, 0.7);");
    this->ui->admin->setStyleSheet("background-color: rgba(12, 60, 85, 0.7);");
    this->ui->data->setStyleSheet("background-color: rgba(12, 60, 85, 0.7);");
}
...
void SwitchRole::on_librarian_clicked()
{
    ...
    this->ui->reader->setStyleSheet("background-color: rgba(12, 60, 85, 0.7);");
    this->ui->admin->setStyleSheet("background-color: rgba(12, 60, 85, 0.7);");
    this->ui->data->setStyleSheet("background-color: rgba(12, 60, 85, 0.7);");
}
...
void SwitchRole::on_continue_2_clicked()
{
    if(this->curUser->is(this->role))
    {
        ...
        if ( this->role == "dataProcessing")
        {

            DataProcessingTechnician *w = new DataProcessingTechnician(0,curUser);
            w->setAttribute(Qt::WA_DeleteOnClose);
            w->show();
            this->close();
            if (parent!=0) parent->close();
