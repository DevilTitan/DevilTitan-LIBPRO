	...
	: QWidget(parent)
{
    ...
    ui.tabWidget->setTabText(0, "Request");
    ui.tabWidget->setTabText(1, "Book");
    ui.Usernamelbl->setAlignment(Qt::AlignCenter);
    this->displaybook();
    this->displayNotify();

    this->setWindowTitle("Data Processor");
}
...
void DataProcessingTechnician::on_searchButton_3_clicked()
{
    ...
    qry.prepare("delete from new_book where idx = '" + QString::number(curidx) + "'");
    if(!qry.exec()) qDebug() << qry.lastError();
    displaybook();
}

void DataProcessingTechnician::on_book_name_textChanged(const QString &cur)
{
    QString search; // xóa khoảng trắng liên tiếp
    for(int i = 1 ; i < cur.size() ; ++i)
    {
    ...
    QString username = curUser->getUsername();
    QSqlQuery query(QSqlDatabase::database("libpro_user"));
    query.prepare("select * from new_book where book_name like '%" + ui.book_name->text() + "%' and "+"username like '%"+ui.username->text()+"%'");
...
void DataProcessingTechnician::on_username_textChanged(const QString &cur)
{
    QString search; // xóa khoảng trắng liên tiếp
    for(int i = 1 ; i < cur.size() ; ++i)
    {
...
void DataProcessingTechnician::on_logout_clicked()
{
    this->close();
...
void DataProcessingTechnician::on_publicChat_3_clicked()
{
    SwitchRole *x = new SwitchRole(this,curUser);
    x->setAttribute(Qt::WA_DeleteOnClose);
    x->setModal(true);
    x->exec();
}

void DataProcessingTechnician::on_theme_clicked()
{
    ChangeTheme * x  = new ChangeTheme(this,curUser);
    x->setAttribute(Qt::WA_DeleteOnClose);
    x->setModal(true);
    x->exec();
}

void DataProcessingTechnician::on_publicChat_6_clicked()
{
    ChangePassword * x = new ChangePassword(0,curUser);
    x->setAttribute(Qt::WA_DeleteOnClose);
    x->setModal(true);
    x->exec();
}

void DataProcessingTechnician::on_publicChat_2_clicked()
{
...
void DataProcessingTechnician::displayNotify()
{
...
void DataProcessingTechnician::updateTime()
{
